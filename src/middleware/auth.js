...
export const protect = async (req, res, next) => {
  try {
    ...
    if (!token) {
      return res.status(401).json({ success: false, message: 'Not authorized: missing token', data: null });
    }
    ...
    if (user.status === 'blocked') {
      return res.status(403).json({ success: false, message: 'Account is blocked', data: null });
    }
